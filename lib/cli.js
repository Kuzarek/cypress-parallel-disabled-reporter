...
const { getTestSuitePaths, distributeTestsByWeight, distributeTestsByTextIncluded } = require('./test-suites');
const {
  formatTime,
  generateWeightsFile,
  collectResults
...
async function start() {
  ...
  if (settings.splitByText.length > 0) {
    threads = distributeTestsByTextIncluded(testSuitePaths, settings.splitByText);
    threads.map(t => {
      t.filteredBy === 'Other tests' ?
        console.log('Running:', t.list.length, t.filteredBy, 'on thread', t.thread) :
        console.log('Running:', t.list.length, 'tests filtered by', t.filteredBy, 'keyword on thread', t.thread)
    })
  } else {
    threads = distributeTestsByWeight(testSuitePaths);
  }
  ...
  // Temporary solution - with reporter disabled we're getting no results
  if (!settings.disableParallelReporter) {
