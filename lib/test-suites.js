...
function distributeTestsByTextIncluded(testSuitePaths, textArray) {
  const testsAssignedToThreads = new Map(textArray.map((t, i) => [t, { filteredBy: t, thread: i, testCount: 0, list: [] }]));
  const unknownPaths = [];

  for (const path of testSuitePaths) {
    const file = fs.readFileSync(path, 'utf8');
    const foundText = textArray.find(t => file.includes(t));
    if (!foundText) {
      unknownPaths.push(path);
      continue;
    }

    const obj = testsAssignedToThreads.get(foundText);
    obj.list.push(path);
    obj.testCount += 1;
  }

  const result = [...testsAssignedToThreads.values()];

  if (unknownPaths.length > 0) {
    result.push({
      filteredBy: 'Other tests',
      thread: textArray.length,
      testCount: unknownPaths.length,
      list: unknownPaths
    });
  }
