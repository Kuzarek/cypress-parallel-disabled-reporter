...
function distributeTestsByTextIncluded(testSuitePaths, text) {
  const testsAssignedToThreads = []

  for (let i = 0; i < text.length; i++) {
    testsAssignedToThreads.push({
      thread: i,
      list: []
    });
  }

  text.forEach((keyword, index) => {
    testSuitePaths.forEach(path => {
      const file = fs.readFileSync(path, 'utf8')
      if (file.includes(keyword)) {
        testsAssignedToThreads[index].list.push(path)
        testSuitePaths.splice(testSuitePaths.indexOf(path), 1)
      }
    })
  })
